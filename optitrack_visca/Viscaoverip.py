...
class Camera:
    ...
    def calculate_pantilt1(self, posx, posy, posz):
        dx_1 = posx - cam1_x
        dy_1 = cam1_y - posy
        dz_1 = cam1_z - posz
        dxy_1 = math.sqrt(dx_1 ** 2 + dy_1 ** 2)

        pan_cam1 = -round((math.degrees(math.atan2(dy_1, dx_1))) * pan_step1)
        tilt_cam1 = -round((math.degrees(math.atan2(dz_1, dxy_1))) * tilt_step1)

        return pan_cam1, tilt_cam1

    def calculate_pantilt2(self, posx, posy, posz):
        dx_2 = cam2_x - posx
        dy_2 = cam2_y - posy
        dz_2 = cam2_z - posz
        dxy_2 = math.sqrt(dx_2 ** 2 + dy_2 ** 2)

        pan_cam2 = round((math.degrees(math.atan2(dy_2, dx_2))) * pan_step2)
        tilt_cam2 = -round((math.degrees(math.atan2(dz_2, dxy_2))) * tilt_step2)

        return pan_cam2, tilt_cam2
    ...
    def pantilt(self, pan_speed: int, tilt_speed: int, pan_position=None, tilt_position=None, relative=False):
        ...
        if position_params.count(None) == 1:
            raise ValueError('Se ha de especificar tanto pan_position como tilt_position o ninguna')

        if abs(pan_speed) > 24 or abs(tilt_speed) > 24:
            raise ValueError('pan_speed y tilt_speed deben estar entre 24 y -24, incluidos')

        if not all(isinstance(param, int) or param is None for param in speed_params + position_params):
            raise ValueError('Todos los parámetros deben ser entero o None')
        ...
        if None not in position_params:
            def encode(position: int):
                """Converts a signed integer to hex with each nibble separated by a 0"""
    ...
    def ir_correction(self, mode: bool):
        ...
        :param mode: True for IR correction mode, False for standard mode
        """
        if mode:
    ...
    def white_balance_mode(self, mode: str):
        ...
        :param mode: One of "auto", "indoor", "outdoor", "auto tracing", "manual", "color temperature", "one push", or
        "one push trigger".
            See the manual for an explanation of these modes.
        """
        modes = {
    ...
    def autoexposure_mode(self, mode: str):
        """Sets the auto exposure mode of the camera
    ...
    def flip_horizontal(self, flip_mode: bool):
        ...
        :param flip_mode: True for horizontal flip mode, False for normal mode
        """
        if flip_mode:
    ...
    def defog(self, mode: bool):
        ...
        :param mode: True for defog mode, False for normal mode
        """
        if mode:
...
posy = float(input("Coordenadas y para las cámaras: "))
posz = float(input("Coordenadas z para las cámaras: "))
pan1_calculado, tilt1_calculado = camarita1.calculate_pantilt1(posx, posy, posz)
pan2_calculado, tilt2_calculado = camarita2.calculate_pantilt2(posx, posy, posz)
print (f"pan1 {pan1_calculado}, tilt1 {tilt1_calculado}")
print (f"pan2 {pan2_calculado}, tilt2 {tilt2_calculado}")
camarita1.zoom_to(0)
camarita2.zoom_to(0)
camarita1.pantilt(24, 24, pan1_calculado, tilt1_calculado, False)
